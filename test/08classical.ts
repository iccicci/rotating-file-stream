...
describe("classical", function() {
	...
	describe("initial rotation with interval", () => {
		const events = test(
			{ files: { "test.log": "test\ntest\n" }, filename: (index?: number | Date): string => (index ? `${index}.test.log` : "test.log"), options: { interval: "1d", rotate: 2, size: "10B" } },
			rfs => {
				rfs.write("test\n");
				rfs.end("test\n");
			}
	...
	describe("rotating on directory which is file", () => {
		const events = test({ files: { txt: "test\n" }, filename: (index?: number | Date): string => (index ? "txt/test.log" : "test.log"), options: { rotate: 2, size: "10B" } }, rfs => {
			rfs.write("test\ntest\n");
			rfs.end("test\n");
		});
