...
describe("compression", () => {
	describe("external", () => {
		...
		it("events", () => deq(events, { finish: 1, open: ["test.log/log", "test.log/log"], rotated: ["test.log/1"], rotation: 1, write: 1 }));
	...
	describe("custom external", () => {
		const events = test(
			{
				filename: (time: Date, index: number) => (time ? `test${index}.log` : "test.log"),
				options:  { compress: (source: string, dest: string): string => `cat ${source} | gzip -c9 > ${dest}`, size: "10B" }
			},
			rfs => rfs.end("test\ntest\n")
		);

		it("events", () => deq(events, { finish: 1, open: ["test.log", "test.log"], rotated: ["test1.log"], rotation: 1, write: 1 }));
		it("file content", () => eq(readFileSync("test.log", "utf8"), ""));
		it("rotated file content", () => eq(gunzipSync(readFileSync("test1.log")).toString(), "test\ntest\n"));
	});

	describe("internal", () => {
		const events = test({ filename: (time: Date, index: number): string => (time ? "log/log/test.gz" + index : "test.log"), options: { compress: "gzip", mode: 0o660, size: "10B" } }, rfs =>
			rfs.end("test\ntest\n")
		);

		it("events", () => deq(events, { finish: 1, open: ["test.log", "test.log"], rotated: ["log/log/test.gz1"], rotation: 1, write: 1 }));
		it("file content", () => eq(readFileSync("test.log", "utf8"), ""));
	...
	describe("error writing tmp file", () => {
		const events = test({ options: { compress: true, size: "10B" } }, rfs => {
			rfs.fsWrite = (fd: number, data: string, encoding: string, callback: (error: Error) => void): void => process.nextTick(() => callback(new Error(`test ${data} ${encoding}`)));
			rfs.end("test\ntest\n");
		});

		it("events", () => deq(events, { close: 1, error: ["test cat test.log | gzip -c9 > 1-test.log utf8"], finish: 1, open: ["test.log"], rotation: 1, write: 1 }));
	});
	...
	describe("error writing and closing tmp file", () => {
		...
		it("events", () => deq(events, { close: 1, error: ["test cat test.log | gzip -c9 > 1-test.log utf8"], finish: 1, open: ["test.log"], rotation: 1, warning: ["test"], write: 1 }));
	});
